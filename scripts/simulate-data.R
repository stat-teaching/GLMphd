...
sim_volunteering <- function(n) {
  ...
  sconnection <- filor::trim(sconnection, 1, 10)
  ...
  X <- model.matrix(
    ~ age0 + sconnection0 + ses + sconnection0:ses,
    data = volunt
  )

  lp <- plogis(as.numeric(X %*% B))
  volunt$vol <- rbinom(nrow(volunt), 1, lp)
  volunt <- volunt[, !endsWith(names(volunt), "0")]
  return(volunt)
}
...
sim_nwords <- function(n) {
  ...
  S <- diag(c(2, 2, 2, 8, 8, 8)) %*%
    (0.6 + diag(1 - 0.6, nrow = 6)) %*%
    diag(c(2, 2, 2, 8, 8, 8))
  xs <- MASS::mvrnorm(
    n,
    mu = c(8, 5, 3, 20, 15, 8),
...
sim_psych <- function(ns, nt) {
  ...
  dat$lp <- (b0 + b0i[dat$id]) +
    (b1 + b1i[dat$id]) * X[, "contrast"] +
    b2 * X[, "condgabor"] +
    b3 * X[, "condface"] +
    b4 * X[, "contrast:condgabor"] +
    b5 * X[, "contrast:condface"]
